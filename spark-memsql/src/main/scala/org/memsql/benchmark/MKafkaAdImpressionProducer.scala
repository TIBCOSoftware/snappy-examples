...
/**
  ...
  */
object MKafkaAdImpressionProducer{
  ...
  val producer = new Producer[String, MAdImpressionLog](config)
  ...
  def sendToKafka(log: MAdImpressionLog) = {
    producer.send(new KeyedMessage[String, MAdImpressionLog](
      MConfigs.kafkaTopic, log.getTimestamp.toString, log))
  }
}
  ...
  def run() {
    for (j <- 0 to numLogsPerThread by maxLogsPerSecPerThread) {
      ...
      for (i <- 1 to maxLogsPerSecPerThread) {
        MKafkaAdImpressionProducer.sendToKafka(MAdImpressionGenerator.nextNormalAdImpression())
      }
