...
import io.snappydata.examples.adanalytics.AdImpressionLog
...
class RabbitMQInputDStream[T: ClassTag](@transient ssc_ : StreamingContext,
  ...
  override def getReceiver(): Receiver[T] = {
    new RabbitMQReceiver[T](params, StorageLevel.fromString(storageLevelParam))
  }
}

class RabbitMQReceiver[T](params: Map[String, String], storageLevel: StorageLevel)
  ...
  private def receive(connection: Connection, channel: Channel) {
    ...
    try {
      ...
      while (!isStopped()) {
        ...
        val decoder = DecoderFactory.get().binaryDecoder(delivery.getBody, null)
        val adLog = reader.read(null, decoder)
        store(adLog.asInstanceOf[T])
